...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const renderRoomCard = (room: Room) => {
    ...
    const grandTotal = currentBooking ? (currentBooking.totalAmount || (roomBaseTotal + addOnsTotal)) : 0;
    ...
    return (
      <div
        ...
        className={`rounded-2xl p-3.5 md:p-4 flex flex-col justify-between cursor-pointer transition-all active:scale-[0.98] bg-white/95 backdrop-blur-md border shadow-[0_4px_16px_rgba(0,0,0,0.03)] hover:shadow-md ${
          isAvailable 
            ...
            : (isOccupied 
                ? (remainingBalance > 0 ? 'border-amber-300 hover:border-amber-400 bg-amber-50/15' : 'border-blue-200/80 hover:border-blue-400 bg-blue-50/10')
                : (isCleaning 
      ...
      >
        <div>
          ...
          {/* Occupied Guest Info & Financial Status Warning */}
          {isOccupied && room.currentGuest && (
            <div className="mt-2 p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-[11px] text-slate-800 space-y-1.5 shadow-2xs">
              <span className="font-black text-slate-900 truncate block flex items-center gap-1">
              ...
              {remainingBalance > 0 ? (
                <div className="p-1.5 rounded-lg bg-amber-100/90 border border-amber-300 text-amber-950 font-bold text-[10px] flex items-center justify-between">
        ...
        <div className="mt-3 pt-2.5 border-t border-slate-100 space-y-1.5">
          ...
          {isOccupied && (
            <div className="flex gap-1.5 flex-wrap">
              {/* If remaining balance > 0, show Record Payment Button */}
              ...
              {/* Check-out Button (Triggers Checkout Modal with payment guard) */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (currentBooking && onOpenCheckoutModal) {
                ...
                className={`py-2 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-1 shadow-sm transition-all ${
                  remainingBalance > 0 
  ...
  return (
    <div className="space-y-4 pb-28 md:pb-8">
      ...
      {/* VIEW 1: L-SHAPED INTERACTIVE RESORT MAP */}
      {viewMode === 'map' && (
      ...
      {viewMode === 'grid' && (
        <div className="space-y-4">
          ...
          {/* SECTION 3: บ้านพักแฝดหลังเล็ก (1,000 บาท/คืน - S5 & S6) */}
          <div className="space-y-2 pt-1">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-xs md:text-sm font-extrabold text-slate-900 flex items-center gap-1.5">
      ...
      {/* Room Detail Modal */}
      {selectedRoomModal && (() => {
        const modalBooking = selectedRoomModal.currentGuest?.bookingId
          ? bookings.find(b => b.id === selectedRoomModal.currentGuest?.bookingId)
          : undefined;

        const roomBase = modalBooking ? modalBooking.roomPrice * modalBooking.totalNights : 0;
        const addOnsSum = modalBooking?.addOns?.reduce((s, a) => s + (a.price * a.quantity), 0) || 0;
        const modalGrandTotal = modalBooking ? (modalBooking.totalAmount || (roomBase + addOnsSum)) : 0;
        const modalRemaining = modalBooking ? Math.max(0, modalGrandTotal - modalBooking.paidAmount) : 0;
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white text-slate-900 w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl p-5 border border-slate-200 shadow-2xl space-y-3.5">
              ...
              <div className="space-y-2 text-xs md:text-sm text-slate-700">
                ...
                {selectedRoomModal.currentGuest && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5 mt-2">
                    ...
                    {/* Financial Summary inside Room Modal */}
                    {modalBooking && (
                      <div className="pt-2 border-t border-slate-200 space-y-1">
                        <div className="flex justify-between text-slate-600">
                          <span>ยอดรวมทั้งสิ้น:</span>
                          <span className="font-black text-slate-900">฿{modalGrandTotal.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-slate-600">
                          <span>ชำระแล้ว:</span>
                          <span className="font-bold text-emerald-700">฿{modalBooking.paidAmount.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between font-black pt-0.5">
                          <span>ยอดคงเหลือ:</span>
                          <span className={modalRemaining > 0 ? 'text-amber-800' : 'text-emerald-700'}>
                            {modalRemaining > 0 ? `฿${modalRemaining.toLocaleString()} (ค้างชำระ)` : 'ชำระครบแล้ว'}
                          </span>
                        </div>
                      </div>
                    )}
              ...
              <div className="pt-2 space-y-2">
                ...
                {selectedRoomModal.status === 'occupied' && modalBooking && (
                  <div className="space-y-2">
                    {/* Record Payment Button */}
                    {modalRemaining > 0 && onOpenAddPayment && (
                      <button
                        onClick={() => {
                          onOpenAddPayment(modalBooking);
                          setSelectedRoomModal(null);
                        }}
                        className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm flex items-center justify-center gap-1.5 shadow-sm active:scale-98"
                      >
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (onOpenCheckoutModal) {
                          onOpenCheckoutModal(modalBooking);
                        } else {
                          onCheckOutGuest(modalBooking.id);
                        }
                      }}
